...
// ------------------------------------------------------------------------- //
    ...
    NS_OBJECT_ENSURE_REGISTERED (GausLogDistancePropagationLossModel);
    
    TypeId
    GausLogDistancePropagationLossModel::GetTypeId (void)
    {
        static TypeId tid = TypeId ("ns3::GausLogDistancePropagationLossModel")
        .SetParent<PropagationLossModel> ()
        .SetGroupName ("Propagation")
        .AddConstructor<GausLogDistancePropagationLossModel> ()
        .AddAttribute ("Exponent",
                       "The exponent of the Path Loss propagation model",
                       DoubleValue (2.5),
                       MakeDoubleAccessor (&GausLogDistancePropagationLossModel::m_exponent),
                       MakeDoubleChecker<double> ())
        .AddAttribute ("ReferenceDistance",
                       "The distance at which the reference loss is calculated (m)",
                       DoubleValue (1.0),
                       MakeDoubleAccessor (&GausLogDistancePropagationLossModel::m_referenceDistance),
                       MakeDoubleChecker<double> ())
        .AddAttribute ("ReferenceLoss",
                       "The reference loss at reference distance (dB). (Default is Friis at 1m with 5.15 GHz)",
                       DoubleValue (46.6777),
                       MakeDoubleAccessor (&GausLogDistancePropagationLossModel::m_referenceLoss),
                       MakeDoubleChecker<double> ())
        .AddAttribute ("GaussianVariable", "The random variable used to pick a loss everytime CalcRxPower is invoked.",
                       StringValue ("ns3::NormalRandomVariable[Mean=0|Variance=0]"),
                       MakePointerAccessor (&GausLogDistancePropagationLossModel::m_Gvariable),
                       MakePointerChecker<RandomVariableStream> ())
        ;
        return tid;
    ...
    GausLogDistancePropagationLossModel::GausLogDistancePropagationLossModel ()
    {
    }
    
    void
    GausLogDistancePropagationLossModel::SetPathLossExponent (double n)
    {
        m_exponent = n;
    }
    void
    GausLogDistancePropagationLossModel::SetReference (double referenceDistance, double referenceLoss)
    {
        m_referenceDistance = referenceDistance;
        m_referenceLoss = referenceLoss;
    }
    double
    GausLogDistancePropagationLossModel::GetPathLossExponent (void) const
    {
        return m_exponent;
    }
    
    double
    GausLogDistancePropagationLossModel::DoCalcRxPower (double txPowerDbm,
                                                    Ptr<MobilityModel> a,
                                                    Ptr<MobilityModel> b) const
    {
        double distance = a->GetDistanceFrom (b);
         ...
         */
        double pathLossDb = 10 * m_exponent * std::log10 (distance / m_referenceDistance);
        double rxc = -m_referenceLoss - pathLossDb + m_Gvariable->GetValue ();
        NS_LOG_DEBUG ("distance="<<distance<<"m, reference-attenuation="<< -m_referenceLoss<<"dB, "<<
                      "attenuation coefficient="<<rxc<<"db, Rando Variable="<< m_Gvariable);
        return txPowerDbm + rxc;
    }
    
    int64_t
    GausLogDistancePropagationLossModel::DoAssignStreams (int64_t stream)
    {
        return 0;
    }
